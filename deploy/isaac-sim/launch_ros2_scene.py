...
CAMERA_CONFIGS = {
    "wrist": CameraCfg(
        prim_path="/World/WristCamMount/wrist_cam",
...
def design_scene():
    ...
    sim_utils.create_prim("/World/WristCamMount", "Xform")
...
def main():
    ...
    # Position cameras after sim.reset() allocates buffers
    device = sim.device
    cameras["wrist"].set_world_poses_from_view(
        eyes=torch.tensor([[0.3, 0.0, 1.3]], device=device),
        targets=torch.tensor([[0.5, 0.0, 1.05]], device=device),
    )
